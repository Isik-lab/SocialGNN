...
# Next 4 functions to decide if 2 given line segments intersect or not: modified version of https://www.geeksforgeeks.org/check-if-two-given-line-segments-intersect/
class Point:
...
def create_graphs_1video_1t(n_entities,t, trajectories, entity_sizes, wall_segs):
    nodes = []
...
def create_Gin(InpVideos):
  ...
  for v in range(len(InpVideos)):
    ...
    for t in range(n_timesteps):   
      graph_dict = create_graphs_1video_1t(n_entities, t, InpVideos[v]['trajectories'], InpVideos[v]['entity_sizes'], InpVideos[v]['wall_segs'])
...
class SocialGNN(object):
  ...
  def _build_graph(self):
    ...
    Gspatial_nodes = gn.blocks.NodeBlock(node_model_fn=lambda: snt.nets.MLP([self.config.V_SPATIAL_SIZE]), use_globals=False)   #REVISIT #n_nodes(4)x timesteps(101/61) x n_videos(20)
...
class SocialGNN_E(object):
  ...
  def _define_inputs(self, sample_graph_dicts_list):
    ...
    #print("Gin_ph", self.Gin_placeholder)
    #print("target_E_ph", self.target_E_placeholder)
  ...
  def _build_graph(self):
    ...
    self.trainable_variables = tf.trainable_variables()
    self.final_state = final_state[0]
